...
export class MarkdownReporter {
  generate(changes: EndpointChange[], fromRef: string, toRef: string, staged = false): string {
    ...
    if (staged) {
      ...
      sections.push(`Comparing \`${formatGitRef(fromRef)}\` and \`${formatGitRef(toRef)}\`\n`);
    }
    ...
    if (modifiedEndpoints.length > 0) {
      sections.push("## Modified Endpoints (Request Parameter Change Suspected)\n");
  ...
  private formatEndpointChange(change: EndpointChange): string {
    ...
    sections.push(`### ${typeLabel} \`${change.functionType}\` for \`${change.endpointPath}\`\n`);
   ...
   */
  generateJson(changes: EndpointChange[], fromRef: string, toRef: string, staged = false): string {
    const report = {
      summary: {
        ...
        modifiedEndpoints: changes.filter((c) => c.type === "param-change").length,
