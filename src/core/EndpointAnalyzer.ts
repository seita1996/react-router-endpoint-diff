...
export class EndpointAnalyzer {
  ...
  analyzeChanges(
    ...
    for (const file of diffFiles) {
      try {
        ...
        logVerbose(`Error analyzing file ${file.newPath || file.oldPath}: ${error}`, verbose);
      }
    }
    ...
    try {
      ...
      if (file.isNew) {
        ...
        for (const func of functions) {
          ...
          const relevantDiff = this.diffParser.getRelevantDiff(originalDiffText, filePath);
      ...
      for (const func of functions) {
        if (this.isFunctionEntirelyNew(func, file.hunks)) {
          ...
          logVerbose(`Found new ${func.type} in existing file: ${urlPath}`, verbose);
        }
      }
    } catch (error) {
    ...
    try {
      ...
      for (const func of functions) {
        if (this.hasParameterChanges(func, file.hunks)) {
          ...
          logVerbose(`Found parameter changes in ${func.type}: ${urlPath}`, verbose);
        }
      }
    } catch (error) {
      logVerbose(`Error detecting parameter changes in ${filePath}: ${error}`, verbose);
    }

    return changes;
  }

  private isFunctionEntirelyNew(func: LoaderActionInfo, hunks: DiffHunk[]): boolean {
    ...
    for (const hunk of hunks) {
      ...
      if (func.startLine <= hunkEndLine && func.endLine >= hunkStartLine) {
        ...
        if (totalLinesInHunk > 0 && addedLinesInFunction / totalLinesInHunk > 0.8) {
          return true;
        }
      }
    }
  ...
  private hasParameterChanges(func: LoaderActionInfo, hunks: DiffHunk[]): boolean {
    ...
    for (const hunk of hunks) {
      ...
      if (func.startLine <= hunkEndLine && func.endLine >= hunkStartLine) {
        ...
        for (const line of hunk.lines) {
          ...
          if (currentLine >= func.startLine && currentLine <= func.endLine) {
            if ((isAddedLine(line) || line.startsWith("-")) && containsRequestParamPattern(line)) {
              return true;
            }
          }
