...
program
  .option("--from <ref>", "Git reference or date expression to compare from", "HEAD~1")
  .option("--to <ref>", "Git reference or date expression to compare to", "HEAD")
  .option("--staged", "Compare staged changes with HEAD", false)
  .option("--routes-dir <path>", "Base directory containing route files", "app/routes")
  .option("--file-pattern <glob>", "File pattern to match route files", "**/*.{ts,tsx,js,jsx}")
