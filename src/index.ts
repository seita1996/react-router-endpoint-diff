...
export class ReactRouterEndpointDiff {
  ...
  async analyze(options: Partial<CLIOptions> = {}): Promise<{
    ...
    try {
      ...
      // Execute git diff
      logVerbose(`Getting diff from ${from} to ${to}${staged ? " (staged)" : ""}`, verbose);
      ...
      logVerbose(`Filtered to ${relevantFiles.length} relevant route files`, verbose);
      ...
      // Generate reports
      const markdownReport = this.markdownReporter.generate(changes, from, to, staged);
