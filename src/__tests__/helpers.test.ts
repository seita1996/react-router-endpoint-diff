...
describe("Utility Functions", () => {
	describe("filePathToUrlPath", () => {
		it("should convert basic file paths to URL paths", () => {
			expect(filePathToUrlPath("app/routes/users.tsx", "app/routes")).toBe(
				"/users",
			);
			expect(
		...
		it("should handle dynamic segments", () => {
			expect(
				filePathToUrlPath("app/routes/users/$userId.tsx", "app/routes"),
			).toBe("/users/:userId");
			expect(
				filePathToUrlPath(
		...
		it("should handle splat routes", () => {
			expect(filePathToUrlPath("app/routes/files/$.tsx", "app/routes")).toBe(
				"/files/*",
			);
		});
		...
		it("should handle nested directories", () => {
			expect(
				filePathToUrlPath("app/routes/admin/users.tsx", "app/routes"),
			).toBe("/admin/users");
			expect(
				filePathToUrlPath("app/routes/api/v1/users.tsx", "app/routes"),
			).toBe("/api/v1/users");
		});
	});
	...
	describe("containsRequestParamPattern", () => {
		it("should detect request parameter patterns", () => {
			expect(containsRequestParamPattern("+const userId = params.userId")).toBe(
				true,
			);
			expect(
				containsRequestParamPattern("+const data = await request.json()"),
			).toBe(true);
			expect(
				containsRequestParamPattern(
			...
			expect(
				containsRequestParamPattern("+const text = await request.text()"),
			).toBe(true);
			expect(
				containsRequestParamPattern('+const query = searchParams.get("q")'),
			).toBe(true);
			expect(
				containsRequestParamPattern('+const name = formData.get("name")'),
			).toBe(true);
			expect(
				containsRequestParamPattern('+const regularCode = "hello world"'),
			).toBe(false);
		});
	});
